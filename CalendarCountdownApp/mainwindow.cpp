...
/*upgradeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee*/
...
void MainWindow::showSelectedDateEvents(const QDate &date) {
    qDebug() << "Selected date clicked:" << date.toString();

    EventDialog eventDialog(this);
    eventDialog.setSelectedDate(date);
    if (eventDialog.exec() == QDialog::Accepted) {
        // 用户确认了事件添加
        events.append(eventDialog.getEvent());
        updateEventList();
    }
}
